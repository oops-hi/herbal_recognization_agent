...
- [x] **二期 V2-A6 云端 VLM 辅助验证**（2026-08-28 落地 → 08-29 真实 key 评测 + 口径修订，默认关闭）：`config.py` 独立 VISION 组（`VISION_API_URL` 默认阿里云百炼 compatible-mode `qwen-vl-max`/`KEY`/`TIMEOUT=5`/`ENABLED` 默认 false）+ `save_vision_config()/vision_state()`（复用 LLM 组热更新模式）；`agent/vision.py`（零 torch 依赖：**灰区三条件** should_use_vision——T1 0.60≤conf<0.90 / T2 混淆对{桃仁,苦杏仁,川楝子,砂仁,豆蔻,草豆蔻,山楂,金樱子} / T3 候选差<0.05；PIL 预处理 ≤1024/≤1.5MB；verify = OpenAI 兼容 image_url base64 + json_object + **结构白名单**（20 类名∪别名归一、verdict∈{none,non_herb}、药性功效词段整段丢弃）+ 429 退避/402/断网 fail-soft；decide 决策矩阵）；`app.py` upload 二段裁决（灰区触发→verify→裁决；`current_herb` 仅放行时写入防带偏）+ `/api/vision-config` GET/POST/test（1x1 像素图连通性测试）；`agent/tools.py` 新增 `vlm_verify` 工具（识药 Agent，`_inject_image_context` 提示同步替换——**顺带修复死提示**：原提示模型调 similar_compare 但该工具不在识药子集）；前端设置页"视觉验证"卡片（URL/Key/模型/开关/测试）+ 识别卡 vision 徽章（绿一致/黄分歧·维持本地/红域外图/灰未参与）；`eval_vision.py` 评测集。⚠️ **实测口径修订（关键）**：qwen-vl-max 域内细粒度弱于本地（砂仁/豆蔻→草豆蔻、苦杏仁→乌梅、金樱子→草豆蔻系统性偏差），域外判定可靠 → **VLM 只在域外拒绝上有否决权**，conflict 不再拒答（弱证据不能否决强证据）：真实评测 **双通道 14/20 = 本地 14/20 不劣化 ✓**（旧口径 5/20 全败）；宠物 6 张（低置信 3 原路径 + 高置信 3：beagle 0.94 木瓜/Bengal 0.89 连翘/boxer 0.91 乌梅）**零放行**——high_bengal 触发 VLM → `non_herb` 域外拒答真实生效；0.90+ 边界 2 张未触发 = 一期已知局限（设计如此，warn 不 fail）；fail-soft 回归 ✓。顺带记录：识药→similar_compare 级联（方案 §8.3）用户已拍板**压后**
