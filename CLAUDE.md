...
│   ├── data/similar_guide.json  # 独立鉴别资料库（17 条两两对，教材汇编 B 档）
...
│   ├── disambiguate.py    # 相似对鉴别闭环：触发检测 + 提问清单/研判材料两模式
...
├── agent/
│   ├── tools.py           # 13 个工具函数 + JSON Schema + 七子 Agent 分组
│   ├── prompts.py         # System Prompt（中文，含溯源/拒答/合规规则）
│   └── core.py            # ReAct 循环 ★核心
├── eval_disambiguate.py   # 相似对鉴别闭环评测（离线 22 + --llm 端到端）
...
5. **知识图谱可视化（v2）**：`/graph` 页 = **D3 手写力导向交互图**（vendor 本地 `static/vendor/d3.v7.min.js`，**页面 0 外部请求**）：拖拽 / 缩放平移 / 悬停邻接高亮 / 点击节点聚焦 + 档案面板联动 / 图例筛选 / 复位；数据源 `GET /api/graph`（`kg.query.graph_dataset()`：98 节点 + 105 边 = 禁忌 9 + 组成 75 + **相似 21**，禁忌/相似双向边各去重为一条，`ensure_ascii=False` 防中文转义）；**失败 / 断网自动降级为 matplotlib 静态 PNG**（`static/kg_graph.png`，离线渲染语义保住）；Windows matplotlib 须设 `plt.rcParams["font.sans-serif"] = ["Microsoft YaHei", "SimHei"]` + `axes.unicode_minus=False`，否则中文全是方框。**第三类边「相似」**（2026-08-31）：琥珀色点线，表示外形易混药材对；由节点档案 `L1.similar_herbs` 在 `builder.py` **派生**（不进 kg.json，validate 边类型白名单无需改），tooltip 悬停显示相似点 + 本品/对比药鉴别对照（互录 points）
...
11. **KV 缓存稳定前缀架构**（2026-08-30）：DeepSeek 前缀缓存**锚定字节 0、前缀精确匹配**（渲染顺序 tools→system→messages，实测换 tools 顺序命中 3712→896）→ ① **tools 恒定全量** `TOOL_SCHEMAS`（不再按子 Agent 过滤，工具纪律由 AGENT_SECTIONS 职责段白名单句约束）；② **BASE 恒定冻结**（去 `{agent}` 占位；⚠️ 后续新规则只进 user 消息尾部或 AGENT_SECTIONS，改 BASE = 全量缓存失效）；③ **历史 append-only**；④ 每轮变化的上下文（子 Agent 职责段/跨对话记忆）由 `core.compose_user_message()` 拼进**本轮新 user 消息**（`[系统指派]` 头部 + `----- 用户问题 -----` 标记）；⑤ 契约单一来源：`ASSIGN_TMPL`/`QUESTION_MARKER` 只在 core.py 定义，app.py title 剥离与 memory 取料共用 `strip_user_wrapper`
12. **跨对话记忆（长对话用户画像提取，Doubao 式）**：`agent/memory.py` + `memory.json`（config.MEMORY_FILE，FROZEN 写 exe 旁，原子写+fail-soft）；schema v2 五类画像（identity 身份/goal 目标/health 健康关注/preference 偏好/fact 事实，kind 白名单校验 + 旧条目缺 kind 归 preference 零迁移）；对话流结束后 daemon 线程画像提炼（节流：会话新增 user 消息 ≥2，`digest_seen` 随会话落盘）；新对话由 `build_context()` 注入本轮 user 消息（kind 标签 + 使用指引：画像影响回答角度，如学生→考点视角；不破坏稳定前缀）。⚠️ **红线：画像只存用户层面信息，禁止记药性/功效/禁忌结论、禁止辨证结论（health 只记自述事实）、禁止编造对话未出现的内容**；API：GET/POST `/api/memory` + DELETE `/api/memory/<id>`，设置页第三张卡管理（五色 kind 标签）；评测 `eval_memory.py`（长对话画像提取 6 项断言，no-op save 隔离用户真实 memory.json）
13. **易混淆饮片鉴别闭环**（2026-08-31，外部资料库 + 识别触发 + 细节提问）：独立鉴别资料库 `kg/data/similar_guide.json`（17 条两两对，来源=用户提供《易混淆中药饮片鉴别汇总》教材汇编，**B 档**标签、不含药典原文；白名单外药材不建档案——绕过 20+ 味新档案人工核对红线，MD 只当「问什么」的内容源）；`kg/disambiguate.py` 三件套——① `find_similar_pairs(top3)`：**两成员都 ∈ top3 才触发**（代码只判同现，是否模糊交给模型，LLM 主裁决红线），连通分量聚簇（砂仁/豆蔻/草豆蔻 → 1 组），触发面 = 图谱互录 21 对 ∪ guide 17 对 ∩ 20 类别 ≈13 对（桃仁/苦杏仁/川楝子、砂仁/豆蔻/草豆蔻、枸杞子/五味子、连翘/栀子、山楂/金樱子 等）② `disambiguate_material(cands, desc)` 两模式：desc 空=**提问清单**（「一看二摸三闻四尝」维度对照表，不强答）/ desc 有=**研判材料**（对照 + 档案节选 + B 档功效差异 + 核对状态，结论等级由模型出：可判定/部分信息/需人工）③ CLI `--pairs-check/--candidates`；`kg/validate.py` 新增 `validate_guide()`（--strict 接入，source 前缀白名单）；触发链路：/upload 成功分支写 `sess["disambig_ctx"]`（tip 文本）→ /chat 透传 `core.stream_run(disambig_ctx=)` → `classify_agent` 新分支（**disambig 激活且非安全/方剂/讲解关键词 → 鉴别**，位于学习分支前：『不是X是Y』『这是什么』都是鉴别流程的一部分）→ `_set_disambig_context` system 注记 4 条硬规则（无特征→取提问清单逐项问；有特征→立即传 desc 研判；无关问题正常答；**优先于【当前识别上下文】**，不得仅凭识别结果直接下结论）；新工具 `disambiguate_similar`（鉴别 Agent 第 3 个工具）；前端 /upload 响应带 `sim_pair` → ChatPanel 以 **assistant 回复样式**提示「可问『这是什么？』」（识别成功与易混提示都走智能体输出样式，不用小字 system-note——用户 08-31 反馈，操作引导类提示仍保留小字）；打包 spec 补 similar_guide.json（whitelist/records 仅构建期用不打包）。**实测**：eval_disambiguate **27/27**（离线 22 + LLM 端到端两轮 5：t1 提问清单工具调用、t2 传 desc 研判 + 结论等级）；validate --strict 0/0 含 guide 17 对；**回归全绿**：eval_safety 10/10、eval_teach --llm 31/31、eval_rag 20/20、npm build
...
- [x] **图谱第三类边「相似」+ 4 组新混淆对**（2026-08-31）：知识侧补 4 组外形易混对（**连翘↔栀子、乌梅↔金樱子、小茴香↔地肤子、覆盆子↔五味子**，全部在分类器 20 类内，points 逐字对照药典【性状】归纳，附加互录不改 whitelist 口径），相似关系 17 → **21 对**（互录 30 味）；图谱侧新增**第三种边**：琥珀色点线 `.link-similar` + 图例「外形相似易混」+ 悬停 tooltip（相似点 + 本品/对比药对照，数据源 = 互录 points）+ 档案面板「相似药材」块 + 离线静态 PNG 同步（viz.py 琥珀点线 + 图例项）；**架构：边由 `builder.py` 从档案派生**（不进 kg.json，validate 边类型白名单/merge 链路零改动）；`graph_dataset()` 105 边 = 禁忌 9 + 组成 75 + 相似 21，全带 reason/points；`similar_compare` 鉴别工具自动获得新对（`--compare 连翘,栀子` 已验）；20 类唯一无相似对的药 = **补骨脂**（后续可补）。validate --strict 0/0、前端 build 通过
- [ ] 待办（答辩前可选）：清理 kg.json 档案出处里残留的「待人工核对」字样（用户已核对完）；「甘草+海藻」口径决策待用户答复（歌诀有、药典未认定的展示口径，review_items 留档）
- [x] **相似对鉴别闭环**（2026-08-31 落地，拍板口径见关键设计 13）：外部资料库 `similar_guide.json`（17 对教材汇编 B 档）+ 识别触发（两者同现）+ 鉴别 Agent 细节提问 + 大模型主裁决（可判定/部分信息/需人工）。**实测**：eval_disambiguate 27/27、validate --strict 0/0、回归全绿（eval_safety 10/10、eval_teach --llm 31/31、eval_rag 20/20、npm build）；**配套顺手修复**：`core.run()` 缺少 `upload_ctx` 透传（拒识上传追问时注记丢失，stream_run 有而 run 没有）——本次随 disambig_ctx 一并补上
...
- [x] **KV 缓存稳定前缀架构 + 跨对话记忆**（2026-08-30，见关键设计决策 11/12）：`prompts.py` BASE 去 `{agent}` 占位冻结 + 抗混淆条 + AGENT_SECTIONS 七段工具白名单句；`core.py` `_ensure_stable_base`/`compose_user_message`/`strip_user_wrapper` + `_chat_completion` 恒定全量 tools + stream_run/run 加 `memory_ctx`（session_stats 透传）；`config.py` MEMORY_* 组；新 `agent/memory.py`（_load/_save 原子写、snapshot/set_enabled/delete_entry/build_context/maybe_digest/_digest_worker，锁 + fail-soft 全覆盖）；`app.py` import + `digest_seen` 会话字段（落盘兼容）+ /chat 注入 memory_ctx + finally `maybe_digest` + `/api/sessions` title 先剥后判空 + GET/POST `/api/memory` + DELETE `/api/memory/<id>`；`demo_agent.py` `print_cache` 命中率打印 + image 仅首轮注入；前端 SettingsView.vue 第三张「跨对话记忆」卡（开关/条目/删除/空态/合规说明）。**实测**：跨 Agent 六连问切换命中率 改造前 ≈0% → 改造后 **43.1→89.1→80.6→90.0→98.0→95.6%**（复测末五轮均值 ≈93%，数字已入答辩要点）；Flask 记忆链路端到端通过（两轮对话→后台摘要落盘 2 条纯偏好条目→第三轮对话引用记忆→DELETE 单条）；**修复过程 bug**：① digest 提示词模板含 JSON 花括号被 `.format` 当占位符抛异常（改 `.replace`）；② json_object 返回的 JSON 在 `choices[0].message.content` 字符串里而非 HTTP 顶层（解析补双层）；③ `time.time_ns()` 同迭代内撞 id 导致 DELETE 一条删两条（id 加序号）；**回归全绿**：eval_safety 10/10（全量 tools 后诊断用例曾失控→职责段补「诊断类禁止调工具」+收敛纪律）、eval_teach --llm 31/31（润肺开放问题曾超轮→讲解段补收敛纪律）、eval_rag 20/20、eval_vision ✓、npm typecheck/build ✓
- [x] **跨对话记忆升级：长对话用户画像提取**（2026-08-31，用户反馈「记忆完全不重要，没有豆包提取身份的能力」后升级，见关键设计决策 12）：schema v2 kind 五类 + `_load` 旧条目零迁移 + DIGEST_PROMPT_TMPL v2（身份优先/重要性过滤/identity·goal 长期保留/禁编造/禁知识结论）+ `build_context` v2（kind 中文标签 + 使用指引）+ `_digest_worker` kind 白名单校验；`prompts.py` 讲解段加考点视角句 + 开放问题收敛纪律（「什么药X」禁止逐味取课文——修 eval_teach 组 C 润肺用例逐味拉整课超配额 + 本草记载误伤）；前端 SettingsView 记忆卡五色 kind 标签；新 `eval_memory.py` **6/6 通过**（8 轮长对话→身份/健康/偏好画像，零知识结论，no-op save 隔离用户真实 memory.json）。**Flask 端到端实测**：两轮对话建画像 → 问「我是什么身份」模型引用画像答学生 → 「讲讲枸杞子」自动考点化并关联眼睛干涩（记忆从装饰变行为）。期间修：摘要幻觉（编造对话未出现的「小茴香」条目→提示词禁编造）。回归：eval_safety 10/10、eval_teach --llm 31/31、润肺用例 3/3 稳定收敛、npm typecheck/build ✓
- [x] **身份即时捕获 + 画像驱动路由**（2026-08-31，用户以「我是一名医学生，要备考，可以多用 study agent 吗」实测反馈）：① `agent/memory.py` `IDENTITY_SIGNAL_PAT`——身份自述单句（「我是…学生/医生」「备考」「考研」）即时触发画像提取，不等攒满 `MEMORY_DIGEST_MIN_NEW`（实测一条身份自述 18 秒后画像即有 identity 条目）；② `agent/core.py` `classify_agent` 加 `memory_ctx` 参数（stream_run 透传）——画像含学生/备考信号（`_STUDY_BOOST_PAT`）时，无关键词命中的单药知识问题从药性兜底**抬到讲解 Agent**，剂量（多少/几克/TEACH_BLOCK_KW）/诊断（`_DIAG_QUESTION_PAT`）类挡回；③ `prompts.py` 讲解段加「身份自述/闲聊直接回应不调工具」。**实测**：eval_memory 扩至 **11/11**（确定性 5/5：身份单句触发 + 路由四断言；LLM 6/6）；Flask 端到端「我是一名医学生，要备考方剂学」单句进画像 → 「枸杞子性味归经是什么」自动路由讲解并答「按考点化结构给你讲解」；回归 eval_safety 10/10、eval_teach --llm 31/31 全绿
- [x] **身份切换修复**（2026-08-31，用户实测「换一个身份就不行了」）：① 摘要提示词加身份更正规则（矛盾新身份 = 删除旧 identity/goal 只留最新，兼容身份允许并存）；② `agent/core.py` 新增 `_has_study_profile()`——boost 信号只扫注入文本的 `-[身份]/-[目标]` 条目行，**不全文匹配**（修复：使用指引样板文本里写死的「如身份为学生」字样让任何画像都命中学生 boost——换成老年人后路由仍走讲解）；③ `IDENTITY_SIGNAL_PAT` 补老年身份词（老年/老人/退休/大爷/大妈/年纪大）。**实测**：学生→老年人换身份端到端通过（画像即刻替换、boost 正确关闭、鉴别 Agent 对话式回应）；eval_memory 扩至 **15/15**（确定性 6/6 + LLM 9/9 含身份切换三断言）；回归 eval_safety 10/10、eval_teach --llm 31/31 全绿
- [x] **易混淆饮片鉴别闭环**（2026-08-31，见关键设计 13）：`kg/data/similar_guide.json`（17 对教材汇编 B 档）+ `kg/disambiguate.py`（两者同现触发 + 连通聚簇 + 提问清单/研判材料两模式）+ `kg/validate.py` `validate_guide()`（--strict 接入）+ 鉴别 Agent 新工具 `disambiguate_similar` + `core` disambig_ctx 全链路（`_set_disambig_context` 4 条硬规则、`classify_agent` 鉴别分支、`stream_run`/`run` 透传）+ app.py 会话 key 三处同步 + /upload `sim_pair` 响应 + ChatPanel assistant 样式提示（识别成功/易混两处，用户反馈改）+ spec 补 similar_guide.json。**实测**：eval_disambiguate 27/27（离线 22 + LLM 端到端两轮 5）、validate --strict 0/0（guide 17 对）、回归全绿（eval_safety 10/10、eval_teach --llm 31/31、eval_rag 20/20、npm typecheck/build ✓）