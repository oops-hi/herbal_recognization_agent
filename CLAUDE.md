...
- [x] **二期 V2-A6 云端 VLM 辅助验证**（2026-08-28，方案 §8.5 全落地，默认关闭）：`config.py` 独立 VISION 组（`VISION_API_URL` 默认阿里云百炼 compatible-mode `qwen-vl-max`/`KEY`/`TIMEOUT=5`/`ENABLED` 默认 false）+ `save_vision_config()/vision_state()`（复用 LLM 组热更新模式）；`agent/vision.py`（零 torch 依赖：**灰区三条件** should_use_vision——T1 0.60≤conf<0.90 / T2 混淆对{桃仁,苦杏仁,川楝子,砂仁,豆蔻,草豆蔻,山楂,金樱子} / T3 候选差<0.05；PIL 预处理 ≤1024/≤1.5MB；verify = OpenAI 兼容 image_url base64 + json_object + **结构白名单**（20 类名∪别名归一、verdict∈{none,non_herb}、药性功效词段整段丢弃）+ 429 退避/402/断网 fail-soft；decide 决策矩阵：consistent 提信/conflict·none 置信不足拒答/non_herb 域外图拒答/unavailable 回退本地）；`app.py` upload 二段裁决（灰区触发→verify→裁决；conflict 本次降级为双通道对比不硬猜——**级联鉴别压后**；`current_herb` 仅 consistent 写入防带偏）+ `/api/vision-config` GET/POST/test（1x1 像素图连通性测试）；`agent/tools.py` 新增 `vlm_verify` 工具（识药 Agent，`_inject_image_context` 提示同步替换——**顺带修复死提示**：原提示模型调 similar_compare 但该工具不在识药子集）；前端设置页"视觉验证"卡片（URL/Key/模型/开关/测试）+ 识别卡 vision 徽章（绿一致/黄分歧·不确定/红域外图/灰未参与）；`eval_vision.py` 评测集（混淆对 20 张 `data/herbs_cls/test/` 8 类取样 + 宠物图 3 张 `tests/pets/` 取自 pet_recognition val + fail-soft 回归）。**无 key 基线已验证**：本地 14/20、fail-soft unavailable 回退放行、宠物 3/3 拒答零放行、GET/POST/test 接口 200、upload 灰区图 `vision.unavailable` 回退本地。⚠️ **待办（用户主责）**：阿里云百炼开通 + 填 `VISION_API_KEY`（或设置页保存开启）→ 跑全量 `eval_vision.py` 出双通道对比（目标 ≥14/20 不劣化、宠物零放行）；顺带记录：识药→similar_compare 级联（方案 §8.3）用户已拍板**压后**
