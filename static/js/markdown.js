...
(function () {
  ...
  function inline(s) {
    ...
    s = s.replace(/(^|[^*])\*([^*\n]+)\*/g, "$1<em>$2</em>");
    s = s.replace(/\x00(\d+)\x00/g, function (_, i) { return codes[+i]; });
    return s;
  }
  ...
  function render(md) {
    ...
    let list = null;      // {tag, items} 连续列表合并成一个 ul/ol
    let codeBuf = null;   // 代码块缓冲（内容不解析 markdown）
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (codeBuf !== null) {
        if (t === "```") { out.push("<pre><code>" + codeBuf.join("\n") + "</code></pre>"); codeBuf = null; }
        else codeBuf.push(esc(ln));
        continue;
      }
      if (t.startsWith("```")) { flushList(); codeBuf = []; continue; }
      ...
      // > 引用
      m = t.match(/^>\s?(.*)$/);
      if (m) { flushList(); out.push("<blockquote>" + inline(esc(m[1])) + "</blockquote>"); continue; }
